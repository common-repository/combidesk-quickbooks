...
namespace Combidesk\QuickBooks;
...
class Application {
	...
	public function __construct() {
		$this->plugin_name         = 'combidesk-quickbooks';
		$this->plugin_product_name = __( 'QuickBooks', $this->plugin_name );
		...
		$this->description = __( 'Synchronize your WooCommerce orders automatically to QuickBooks', $this->plugin_name );
		$this->run();
	}
	...
	public function woo_hpos_compatibility() {
		if ( class_exists( \Automattic\WooCommerce\Utilities\FeaturesUtil::class ) ) {
			\Automattic\WooCommerce\Utilities\FeaturesUtil::declare_compatibility( 'custom_order_tables', 'combidesk-quickbooks/combidesk-quickbooks.php', true );
		}
	}
	...
	function add_combidesk_page() {
		...
		$menu_slug  = 'combidesk-quickbooks-settings';
	...
	function combidesk_settings() {
		...
		echo '
		  ...
		  <form class="combidesk__cta-form" action="https://combidesk.com/koppeling/woocommerce-quickbooks" method="GET" target="_BLANK">
		  ...
		  <form class="combidesk__cta-form" action="https://refer.quickbooks.co.uk/tim9+955455b66c!a" method="GET" target="_BLANK">
	...
	function load_combidesk_plugin_style( $hook ) {
		if ( $hook != 'toplevel_page_combidesk-quickbooks-settings' ) {
			return;
		}
		
		wp_enqueue_style( 'custom_wp_admin_css', plugins_url( '../assets/scss/combidesk.min.scss', __FILE__ ) );
	}
}
